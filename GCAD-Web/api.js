...
function request(req, res) {
    ...
    if (!action) {
            ...
            if (req.query.password != main.APIPassword) {
                ...
                for (var i = 0; i < json.table.length; i++) {
                    if (fs.existsSync(`data/${json.table[i].username}/civillians.json`)) {
                        ...
                        civs.forEach(civ => {
                            if (civ.Firstname.toLowerCase() == civInfo[0].toLowerCase() && civ.Surname.toLowerCase() == civInfo[1].toLowerCase()) {
                                foundCiv = true
                                res.send(JSON.stringify(civ))
                                return;
                            }
                        })
                    }
                ...
                if (!foundCiv) {
                    res.send("could not find supplied civ name")
                    res.end()
                    return;
                }
            }
        }
    } else if (action == "licensePlate") {
        if (!req.query.password || !req.query.plate) {
            res.send("missing args for licensePlate check. Need login and plate")
            res.end()
            return
        } else {
            if (req.query.password != main.APIPassword) {
                res.send("API password incorrect")
                res.end()
                return
            } else {
                var json = main.jsonRead("data/users.json")
                ...
                json.table.forEach(user => {
                    if (fs.existsSync(`data/${user.username}/vehicles.json`)) {
                        var vehicles = main.jsonRead(`data/${user.username}/vehicles.json`)
                        vehicles.forEach(vehicle => {
                            if (vehicle.plate == req.query.plate) {
                                plateFound = true;
                                res.send(vehicle)
                                res.end()
                                return
                ...
                if (!plateFound) {
                    res.send("no results in database")
                    res.end()
                    return
                }
            }
        ...
        if (!req.query.username || !req.query.password) {
            ...
            var found = false
            json = main.jsonRead("data/users.json")
            json.table.forEach(user => {
            ...
            if (!found) {
                var encryptedPass = bcrypt.hashSync(MD5(req.query.password), main.saltRounds)
                var newObject = {
                ...
                main.jsonWrite(json, "data/users.json")
    ...
    else if(action == "code99"){
        var roadName = req.query.roadName 
        console.log(roadName)
        var callNumber = main.callAmmount()
        var newObject = {
            "callID": callNumber,
            "Code10": "CODE99",
