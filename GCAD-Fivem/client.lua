...
AddEventHandler("addBlip", function(source)
    ...
    local roadHash, crossingRoadHash = GetStreetNameAtCoord(GetEntityCoords(GetPlayerPed(-1)).x, GetEntityCoords(GetPlayerPed(-1)).y, GetEntityCoords(GetPlayerPed(-1)).z)
    local roadName = GetStreetNameFromHashKey(roadHash)
    print(roadName)
    ...
    RageUI.IsVisible(menu, true, true, true, function()
                ...
                if name then
                    ...
                    if count > 2 then
                        ...
                        local args = {
                            ...
                            WebsiteLink .. "/api?action=personSearch&password=" ..
        ...
        RageUI.Button("Vehicle database", "serch up license plates in the cad",
            ...
            if (Selected) then
                ...
                local args = {
                    GetPlayerServerId(PlayerId()),
                    WebisteLink.."/api?action=licensePlate&password="..APIKey.."&plate=" ..
                        plate, "plate"
                }
                TriggerServerEvent("performHTTPGET", args)
                RageUI.CloseAll()
...
AddEventHandler("WarrantsRemoved", function() 
    local nameArray = string.split(name, " ", 99)
    if #nameArray == 2 then
        local args = {GetPlayerServerId(PlayerId()), WebisteLink.."/api?action=personSearch&password="..APIKey.."&name="..nameArray[1].."%20"..nameArray[2], "person"}
        TriggerServerEvent("performHTTPGET", args)
    end
end)
